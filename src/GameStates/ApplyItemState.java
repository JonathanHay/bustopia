 ...
 * @author Jonathan
 */
public class ApplyItemState extends GameState {

    private Background background;
    private HayButton goBack;
    private HayButton apply;
    ...
    public ApplyItemState(StateManager sm) {
        this.sm = sm;
    ...
    public void init() {
        ...
        apply = new HayButton("Resources/MenuBackground/button1.png", "Resources/MenuBackground/button1down.png", "Install", OtherConstants.DEFAULT_FONT, 1, 1);
        apply.setPosition(440, 25);
    ...
    public void draw(Graphics2D g) {
        ...
        if (itemSelected && currentChoice >= 0) {
            ...
            if (Inventory.applyChoice == OtherConstants.APPLYENGINE) {
                g.drawString("->" + String.valueOf(Inventory.applyEngine.getToughness()), 520, 180);
                g.drawString("->" + String.valueOf(Inventory.applyEngine.getEfficiency()), 520, 190);
            } else if (Inventory.applyChoice == OtherConstants.APPLYWHEELS) {
                g.drawString("->" + String.valueOf(Inventory.applyWheels.getToughness()), 520, 220);
                g.drawString("->" + String.valueOf(Inventory.applyWheels.getSpeed()), 520, 230);
            } else if (Inventory.applyChoice == OtherConstants.APPLYFUELTANK) {
                g.drawString("->" + String.valueOf(Inventory.applyTank.getToughness()), 520, 260);
                g.drawString("->" + String.valueOf(Inventory.applyTank.getFuelTankSize()), 520, 270);
            }
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (apply.checkClick(mBox) && currentChoice >= 0) {
            if (Inventory.applyChoice == OtherConstants.APPLYENGINE) {
                Inventory.playerEngines.set(Inventory.applyType, Inventory.playerBuses.get(currentChoice).getEngine());
                Inventory.playerBuses.get(currentChoice).setEngine(Inventory.applyEngine);
                sm.setState(StateManager.INVENTORY);
            } else if (Inventory.applyChoice == OtherConstants.APPLYWHEELS) {
                Inventory.playerWheels.set(Inventory.applyType, Inventory.playerBuses.get(currentChoice).getWheels());
                Inventory.playerBuses.get(currentChoice).setWheels(Inventory.applyWheels);
                sm.setState(StateManager.INVENTORY);
            } else if (Inventory.applyChoice == OtherConstants.APPLYFUELTANK) {
                Inventory.playerTanks.set(Inventory.applyType, Inventory.playerBuses.get(currentChoice).getFuelTank());
                Inventory.playerBuses.get(currentChoice).setFuelTank(Inventory.applyTank);
                sm.setState(StateManager.INVENTORY);
            }
        }
